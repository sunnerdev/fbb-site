...
function checkLoginStatus() {
    ...
    if (loginCookie === null || loginCookie === undefined || loginCookie === "") {
        if (loginStatus !== '' && loginStatus !== 'empty') {
            if (loginStatus === 'multiple') {
                window.location.href = window.location.href + "#multiple";
                location.reload(true);
            } else {
                console.log(String.format("logout.aspx triggered. loginCookie: {0} loginStatus: {1}", loginCookie, loginStatus));
...
function load_casino_link_v2(code, queryStr) {
    queryStr = queryStr || '';

    if (loginStatus) {
        try { casino_V2_window.close(); } catch (e) { }
        casino_V2_window = window.open("/Casino/launcher.aspx?" + ((code) ? code : "") + queryStr, 'casino');
    }
    else { showLogin(); }
}
...
function loadSuperBullWithTrial(product, trial) {
    if (trial)
        return open_launcher(String.format("/Superbull/Launcher?provider=superbull&game={0}&t=1", product), "superbull");
    else
        loginStatus ? open_launcher(String.format("/Superbull/Launcher?provider=superbull&game={0}&t=0", product), "superbull") : showLogin();
}
...
function loadP2PGame(product, trial) {
    ...
    open_launcher(String.format("/P2P/Launcher?provider=p2p&game={0}&t=0", product), "p2p");
}

function loadP2PLGGame(product, trial) {
    if (!loginStatus)
        return showLogin();

    open_launcher(String.format("/P2P/Launcher?provider=lg&game={0}&t=0", product), "p2p");
}
...
function loadPokerGame(product, trial) {
    ...
    open_launcher(domain_www + "/Poker/Launcher?provider=poker&game=" + product + "&t=0", "poker");
}

function loadDominoQQGame(product, trial) {
    if (trial)
        return open_launcher(domain_www + "/Domino/Launcher?provider=dominoqq&game=" + product + "&t=1", "dominoqq");
    else
        loginStatus ? open_launcher(domain_www + "/Domino/Launcher?provider=dominoqq&game=" + product + "&t=0", "dominoqq") : showLogin();
}
...
function loadP2PTexasHoldemPoker(product, trial) {
    if (trial)
        return open_launcher(domain_www + "/P2P/Poker/Launcher?provider=texasholdempoker&game=" + product + "&t=1", "texasHoldemPoker");
    else
        loginStatus ? open_launcher(domain_www + "/P2P/Poker/Launcher?provider=texasholdempoker&game=" + product + "&t=0", "texasHoldemPoker") : showLogin();
}

function loadP2PTienLen(product, trial) {
    if (trial)
        return open_launcher(domain_www + "/P2P/TienLen/Launcher?provider=tienlen&game=" + product + "&t=1", "tienlen");
    else
        loginStatus ? open_launcher(domain_www + "/P2P/TienLen/Launcher?provider=tienlen&game=" + product + "&t=0", "tienlen") : showLogin();
}

function loadP2PPokDeng(product, trial) {
    if (trial)
        return open_launcher(domain_www + "/P2P/PokDeng/Launcher?provider=pokdeng&game=" + product + "&t=1", "pokDeng");
    else
        loginStatus ? open_launcher(domain_www + "/P2P/PokDeng/Launcher?provider=pokdeng&game=" + product + "&t=0", "pokDeng") : showLogin();
}

function loadP2PZ88Game(product, trial) {
    if (!loginStatus)
        return showLogin();

    open_launcher(String.format("/P2P/Launcher?provider=z88&game={0}&t=0", product), "z88");
}
...
//set loginStatus from meta.ascx
...
function getRealIp() {
    var udata = utility.getFromStorage(true, "udata");
    if (!$.cookie("clientIp")) {
        utility.DoAjaxGetJsonp(udata.ip2locUrl,
            null,
            null,
            function (data) {
                if (data && data.ip)
                    $.cookie("clientIp", data.ip);
            });
    }
};
...
function getCurrentAvgWithdrawDepositTime(successCallBack, failedCallBack) {
    var udata = utility.getFromStorage(true, "udata");
    var headers = { Authorization: 'Bearer ' + window.ftkn };
    var url = String.format("/_secure/ajax/api/member/currentAvgWithdrawDepositTime/{0}", udata.lang);
    utility.DoAjaxGet(url, null, headers, successCallBack, failedCallBack);
};

function getHotMatches(successCallBack, failedCallBack) {
    var udata = utility.getFromStorage(true, "udata");
    var headers = { Authorization: 'Bearer ' + window.ftkn };
    var url = String.format("/_secure/ajax/api/member/hotmatches/{0}/all", udata.lang);
    utility.DoAjaxGet(url, null, headers, successCallBack, failedCallBack);
};

function getHotMatchesByLanguage(successCallBack, failedCallBack) {
    var udata = utility.getFromStorage(true, "udata");
    var headers = { Authorization: 'Bearer ' + window.ftkn };
    var url = String.format("/_secure/ajax/api/member/hotmatches/{0}", udata.lang);
    utility.DoAjaxGet(url, null, headers, successCallBack, failedCallBack);
};
 ...
 */
(function (window) {
	var hostUrl = window.useLocalHost ? 'http://localhost:61739/api/pushy' : '/_secure/ajax/api/pushy';

	this.pushyService = new function () {
		var self = this;

		self.sendMessage = function (msg, url, id, successCallBack, failedCallBack) {
		    var headers = { Authorization: 'Bearer ' + window.ftkn },
                udata = utility.getFromStorage(true, "udata"),
                lang = udata.lang === 'zh-my' ? 'zh-cn' : udata.lang;

			if (msg && msg.length > 0) {
				var host = String.format("{0}/notify", hostUrl);
				var data = {
					"to": id,
					"data": {
						"message": msg,
				...
				utility.DoAjaxPost(host, data, headers, function (d) {
					if (successCallBack && typeof successCallBack === 'function')
						successCallBack(d);
				},
				function (e) {
		...
		self.subscribeToTopics = function (topics, url, id, successCallBack, failedCallBack) {
		    var headers = { Authorization: 'Bearer ' + window.ftkn },
                udata = utility.getFromStorage(true, "udata"),
                lang = udata.lang === 'zh-my' ? 'zh-cn' : udata.lang;

			if (topics && Array.isArray(topics) && topics.length > 0) {
				var host = String.format("{0}/subs", hostUrl);
				var data = {
					"token": id,
					"topics": topics
				};
		...
		self.unSubscribeToTopics = function (topics, url, id, successCallBack, failedCallBack) {
		    var headers = { Authorization: 'Bearer ' + window.ftkn },
                udata = utility.getFromStorage(true, "udata"),
                lang = udata.lang === 'zh-my' ? 'zh-cn' : udata.lang;

			if (topics && Array.isArray(topics) && topics.length > 0) {
				var host = String.format("{0}/unsubs", hostUrl);
				var data = {
					"token": id,
					"topics": topics
				};
		...
		self.getTopicsFromBo = function (successCallBack, failedCallBack) {
		    var headers = { Authorization: 'Bearer ' + window.ftkn },
                udata = utility.getFromStorage(true, "udata"),
                lang = udata.lang === 'zh-my' ? 'zh-cn' : udata.lang;

			var host = String.format("{0}/bo/topics/all", hostUrl);

			utility.DoAjaxGet(host, null, headers,
				function (d) {
					if (successCallBack && typeof successCallBack === 'function')
						successCallBack(d);
				}, function (e) {
		...
		self.getInfo = function (deviceId, successCallBack, failedCallBack) {
		    var headers = { Authorization: 'Bearer ' + window.ftkn },
                udata = utility.getFromStorage(true, "udata"),
                lang = udata.lang === 'zh-my' ? 'zh-cn' : udata.lang;

			var host = String.format("{0}/device/{1}/info", hostUrl, deviceId);
